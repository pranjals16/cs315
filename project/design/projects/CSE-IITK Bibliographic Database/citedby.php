...
$basic_query = "SELECT DISTINCT PTitle, AuthName, Year, RName, CName, DOI, AuthMail, Venue , PId, PRank FROM (Paper natural join Auth natural join Author natural join PublishedIn natural join Conference natural join BelongsTo natural join ResearchArea natural join Keyword),Cites WHERE ToPId = $pid AND FromPId = Paper.PId ";
...
$paper = mysql_fetch_array(mysql_query("SELECT PTitle from Paper where PId = $pid",$db));
echo "<span class = h1>Papers citing \"<i>".$paper[0]."</i>\"</span><br><br>";
$result = mysql_query($basic_query.$end_query);
...
$result_auth = mysql_query($basic_query." GROUP BY AuthName");
...
$result_area = mysql_query($basic_query." GROUP BY RName");
...
$_SESSION['query_torefine'] = $basic_query;
